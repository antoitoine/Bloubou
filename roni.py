...
roni.setCommand(0, musiqueYoutube, r"(?:joue[r|s]?|play|chante[r|s]?|mets?(?:tr?es?)?|d[e|é]marres?|commences?) +(?:la|les?|une?s?)? *(?:musiques?|chansons?|sons?)? *(?P<lien>.+)")
roni.setCommand(1, pause, r"pauses?")
roni.setCommand(2, resume, r"(?:resumes?|repren[d|t]s?|continues?)")
roni.setCommand(3, disconnect, r"(?:(?:re)?par[s|t]|partir|bouges?|va-t'en|d[e|é]co|d[e|é]gages?)")
roni.setCommand(4, stop, r"(?:stop|arr[ê|e]tes? +(?:la|les?) +(?:musiques?|chansons?|sons?))")
roni.setCommand(5, viensChannel, r"(?:vien[s|t]|revien[s|t]|venir|ram[e|è]nes?(?:[ |-]toi)?)")
roni.setCommand(6, vaChannel, r"(?:vas? *)?(?:(?:plus )?vite|vas?|voir|rejoindre|rejoin[t|s]?|connecte[s|r]?-? *(?:toi)??|aller|go) +(?:(?:avec|voir) +|(?:dans +)?le +(?:salon(?: vocal)?|voc(?:al)?|channel) +(?:de +|d'))?(?P<user>.+?(?=(?:\n| +ou +(?:d'|de )?(?P<other>.+)| +et| +il| +elle)))")
